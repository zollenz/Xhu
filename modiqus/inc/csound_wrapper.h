...
EXTERN_C MYFLT *mq_get_channel_pointer(const char *name, mq_s32_t flags);
EXTERN_C MYFLT mq_get_control_channel_value(MYFLT *channel);
...
EXTERN_C const mq_s32_t mq_get_table_data(const mq_s32_t tableNumber, audio_data_t* const data);
EXTERN_C void mq_set_table_data(const mq_s32_t table, const audio_data_t *const data, mq_u32_t data_count);
